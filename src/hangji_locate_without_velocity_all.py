...
"""
===========================
@Time : 2023/12/17 17:57
@Author : karsten
@File : hangji_locate_without_velocity_all.py
@Software: PyCharm
============================
"""
# !/usr/bin/env python3
...
for root, dirs, files in os.walk(event_path):
	for dir in dirs:
		...
		for circle in range(stop_circle):
			G, d, W, corr = locate_fun.build_matrices_without_velocity(rel_coordinates, stp, E0, V0, sigma_t, maxlag, sample_rate)
			delta_m = np.linalg.pinv(W @ G) @ W @ d  # 使用伪逆来求解
			# 更新地震位置估计
			E0 = E0 + delta_m[:2].flatten()  # 确保delta_m是一维的
			V0 = V0 + velocity_rate * delta_m[2]
			...
			if residual > R[circle] or np.abs(residual - R[circle]) < stop_critical:
				...
				# 打印或记录信息
				print(f"quake_name:{dir},Iteration {circle}: Delta M = {delta_m.T}, New E0 = {E0}, New V0 = {V0}, Residual = {residual}")
		event_locate.append([dir, E[-1][0], E[-1][1], V[-1][0], R[-1]])
...
event_locate_df.columns = ['event_name','location_x', 'location_y', 'velocity','residual']
# 保存并不带序号
event_locate_df.to_csv(os.path.join(output_path, 'event_locate_without_velocity.csv'), index=False)