...
"""
===========================
@Time : 2023/12/23 11:15
@Author : karsten
@File : hangji_locate_Tikhonov_Regularization_beta.py.py
@Software: PyCharm
============================
"""
import numpy as np
...
for b_eta in beta:
	...
	for circle in range(stop_circle):
		...
		# 加入正则化项
		delta_m = (np.linalg.inv(J.T @ J + alpha ** 2 + b_eta ** 2 * L.T @ L) @
				   (-J.T @ (G_m - d) - alpha ** 2 * (vk - v0) - b_eta ** 2 * L.T @ L @ m))
		# 更新m
		m = m + delta_m
		# 计算新的G与Vk
		for k in range(K):
		...
		print(
			f"alpha:{alpha}, beta:{b_eta},Iteration {circle}: norm(Delta M) = {np.linalg.norm(delta_m)}, Residual = {residual[-1]}")
...
for i in range(len(G_m_d_record)):
	...
	# 为每个点添加文本
	plt.text(G_m_d_record[i], v_length[i], beta[i])
...
plt.savefig('/Users/karsten_hkt/PycharmProjects/seismo_live_local/obspy_learning/output/all_G_m_without_velocity_beta.jpg',dpi=300)